...
export default function Boost({ currentView, setCurrentView }: BoostProps) {
  ...
  //   if (energyRefillsLeft > 0) {
  //     simulateServerRequest(() => {
  //       refillEnergy();
  //       showSuccessMessage('Energy refilled!');
  //     }, setIsLoadingRefill);
  //   } else {
  //     showErrorMessage('No refills left for today!');
  //   }
  // };
  ...
  const handleEnergyRefill = async () => {
    if (isEnergyRefillAvailable) {
      ...
      try {
        ...
        showSuccessMessage('Energy Refill Successful!');
      } catch (error) {
        console.error('Error refilling energy:', error);
        showErrorMessage('Failed to refill energy. Please try again.');
      } finally {
  ...
  //   simulateServerRequest(() => {
  //     upgradeMultitap();
  //     showSuccessMessage('Multitap upgraded!');
  //   }, setIsLoadingMultitap);
  // };
  ...
  const handleMultitapUpgrade = async () => {
    if (isMultitapAffordable && !isLoadingMultitap) {
      ...
      try {
        ...
        showSuccessMessage('Multitap Upgrade Successful!');
      } catch (error) {
        console.error('Error upgrading multitap:', error);
        showErrorMessage('Failed to upgrade multitap. Please try again.');
      } finally {
  ...
  //   simulateServerRequest(() => {
  //     upgradeEnergyLimit();
  //     showSuccessMessage('Energy limit upgraded!');
  //   }, setIsLoadingEnergyLimit);
  // };
  ...
  const handleEnergyLimitUpgrade = async () => {
    if (isEnergyLimitAffordable && !isLoadingEnergyLimit) {
      ...
      try {
        ...
        showSuccessMessage('Energy Limit Upgrade Successful!');
      } catch (error) {
        console.error('Error upgrading energy limit:', error);
        showErrorMessage('Failed to upgrade energy limit. Please try again.');
      } finally {
